...
class TransformerForDiffusion(ModuleAttrMixin):
    ...
    def forward(self, 
        ...
        time_emb = self.time_emb(timesteps).unsqueeze(1) # B -> B*1*n_emb
        # (B,1,n_emb)
        ...
        if self.encoder_only:
            ...
            if self.obs_as_cond:
                ...
                # (B,To,n_emb)
                cond_embeddings = torch.cat([cond_embeddings, cond_obs_emb], dim=1) # B,T+To,n_emb 
...
if __name__ == '__main__':
    ...
    timestep = torch.tensor(0)
    sample = torch.zeros((4,8,16)) # batch,horizon,input_dim 也是输入 noisy actions
    cond = torch.zeros((4,4,10)) # batch,n_obs_steps,cond_dim
    out = transformer(sample, timestep, cond)
    print(out)