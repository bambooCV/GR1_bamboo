...
class GR1CalvinEvaluation(CalvinBaseModel):
    ...
    def step(self, obs, goal,step = 0,diff_flag = False, debug=False):
        ...
        re_diff_set = 200
        # 大策略
        # if step == 0 or step == 30 or step%re_diff_set == 0 or diff_flag == True or self.strategy_flag == True:
            ...
            if sum(variance)<0.001: # 方差过小，则采用小策略
                ...
                if self.reset_point:
                    ...
                    if self.leave_reset_num > 20:
                        ...
                        self.leave_reset_num = 0
        # 小策略 防止生成2d_diff的时候没有看到目标物体：柜里的目标和move slider right
        ...
        # if "sliding cabinet" in text and "lift" in text:
        #     if step == 0:
        #         re_out_action = torch.zeros((30, 2))
        #         re_out_action[:,0] = re_out_action[:,0] + 120# x
        #         re_out_action[:,1] = re_out_action[:,1] + 145# y
        #         self.traj_2d_list = []
        #         for _ in range(10):
        #             self.traj_2d_list.append(re_out_action.squeeze(0))
        #     if step == 30: 
